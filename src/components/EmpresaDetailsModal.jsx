...
const EmpresaDetailsModal = ({ empresa, onClose }) => {
  ...
  const badgeClass = (colorClass) =>
    `inline-block px-2 py-1 rounded text-xs font-semibold ml-2 ${colorClass}`;
  ...
  useEffect(() => {
    ...
    api
      .get(`/lojas?empresaId=${empresa.id}`)
    ...
    api
      .get(`/usuarios?empresaId=${empresa.id}`)
  ...
  async function handlePlanoChange(e) {
    ...
    setMsg(null);
    try {
      await api.patch(`/superadmin/empresas/${empresa.id}/plano`, {
        plano: novoPlano,
      });
  ...
  async function handleAtivarDesativar() {
    ...
    setMsg(null);
    try {
      await api.patch(`/superadmin/empresas/${empresa.id}/ativo`, {
        ativo: !ativo,
      });
      setAtivo((a) => !a);
    ...
    >
      ...
      >
        ...
        <div className="mb-2 flex items-center">
          ...
          {loading && (
            <span className="ml-2 text-blue-600 text-sm">Salvando...</span>
          )}
        </div>
        ...
        <div className="mb-2">
          <b>Data Criação:</b> {empresa.dataCriacao}
        </div>
        <div className="mb-2">
          <b>Data Atualização:</b> {empresa.dataAtualizacao}
        </div>

        {msg && (
          <div
            className={`mb-4 font-semibold ${msg.includes("sucesso") ? "text-green-600" : "text-red-600"}`}
          >
            {msg}
          </div>
        )}
